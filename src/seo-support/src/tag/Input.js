...
export default class Input extends React.Component {
	...
	render(){
		var {type, onChange, onFocus, onBlur, ...props} = this.props;
		...
		props.onFocus = (e)=>{
			this.scrollIntoViewHandler = ()=>{
				...
				if(this.refs.input){
					this.scrollIntoView = scrollIntoView(this.refs.input);
				}
			};
			...
			if(Screen.size.height === screenHeight && document.body.scrollTop === 0){
				this.scrollToStart = setTimeout(()=>{
					if(this.refs.input && Screen.size.height === screenHeight && document.body.scrollTop === 0){
						this.scrollIntoView = scrollIntoView(this.refs.input, true);
					}
				}, 500);
			}
